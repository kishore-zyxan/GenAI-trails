...
client = openai.OpenAI(
    api_key="11daa5c2-5ff5-482a-a204-be55488e5de7",
...
# Optional: Flatten nested JSON if model still returns nested
def flatten_json(y):
    out = {}
    def flatten(x, name=''):
                ...
                with st.spinner("Sending to SambaNova for analysis..."):
                    try:
                        ...
                        try:
                            ...
                            json_data_flat = flatten_json(json_data)

                            st.subheader(f"🧠 Extracted Key-Value Pairs from {uploaded_file.name}:")
                            st.json(json_data_flat)

                            # Insert into MySQL
                            insert_into_mysql(uploaded_file.name, json_data_flat)
